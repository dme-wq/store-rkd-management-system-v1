...
function parseDate(ts: string): Date {
  ...
  // Handle "14 March 2026, 17:31:51" style
...
export default function ReportPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (selDept    !== "__all__") result = result.filter(r => r["Department"]         === selDept);
    if (selMachine !== "__all__") result = result.filter(r => r["Machine Name"]       === selMachine);
    if (selMachineID !== "__all__") result = result.filter(r => r["Machine ID"]       === selMachineID);
    if (selItem    !== "__all__") result = result.filter(r => r["Item Name"]          === selItem);
    if (selPerson  !== "__all__") result = result.filter(r => r["Person Filling Name"] === selPerson);
    if (selRKD     !== "__all__") result = result.filter(r => r["Store RKD Number"]   === selRKD);
  ...
  // ── Table Columns ─────────────────────────────────────────────────────
  const COLS: { label: string; key: string; className?: string }[] = [
    { label: "#",             key: "_sno"             },
    { label: "Timestamp",    key: "Timestamp",        className: styles.tdMono },
    { label: "Department",   key: "Department"        },
    { label: "Item Name",    key: "Item Name",        className: styles.tdBold },
    { label: "Machine Name", key: "Machine Name"      },
    { label: "Machine ID",   key: "Machine ID",       className: styles.tdMono },
    { label: "Request No.",  key: "Store RKD Number"  },
    { label: "Person",       key: "Person Filling Name" },
    { label: "Indent Qty",   key: "Require Qty"       },
    { label: "Issue Qty",    key: "Issue Qty",        className: styles.tdQty },
    { label: "Rate",         key: "Price",            className: styles.tdPrice },
    { label: "Total Price",  key: "Total Price",      className: styles.tdTotal },
  ];
        ...
        <div className={styles.kpiRow}>
          <div className={`${styles.kpiCard} ${styles.kpiPurple}`}>
            ...
            <div className={styles.kpiText}>
              <div className={styles.kpiValue}>{kpis.count.toLocaleString("en-IN")}</div>
              <div className={styles.kpiLabel}>Total Entries</div>
              <div className={styles.kpiSub}>Requirement Closed</div>
            </div>
          </div>

          <div className={`${styles.kpiCard} ${styles.kpiCrimson}`}>
            <div className={styles.kpiIconBox}>📦</div>
            <div className={styles.kpiText}>
              <div className={styles.kpiValue}>{kpis.totalIssueQty.toLocaleString("en-IN")}</div>
              <div className={styles.kpiLabel}>Total Issue Qty</div>
              <div className={styles.kpiSub}>Items issued from store</div>
            </div>
          </div>
          ...
          <div className={`${styles.kpiCard} ${styles.kpiGold}`}>
            <div className={styles.kpiIconBox}>📝</div>
            <div className={styles.kpiText}>
              <div className={styles.kpiValue}>{kpis.totalIndentQty.toLocaleString("en-IN")}</div>
              <div className={styles.kpiLabel}>Total Indent Qty</div>
              <div className={styles.kpiSub}>Items requested</div>
            </div>
          </div>
          ...
          <div className={`${styles.kpiCard} ${styles.kpiTeal}`}>
            ...
            <div className={styles.kpiText}>
              <div className={styles.kpiValue}>
                {kpis.totalPrice >= 100000
                  ? "₹" + (kpis.totalPrice / 100000).toLocaleString("en-IN", { maximumFractionDigits: 1 }) + "L"
                  : fmtCurrency(kpis.totalPrice)}
              </div>
              <div className={styles.kpiLabel}>Grand Total Price</div>
              <div className={styles.kpiSub}>{fmtCurrency(kpis.totalPrice)}</div>
            </div>
          </div>
        </div>
          ...
          <div className={styles.tableWrap}>
            {loading ? (
              ...
              <table className={styles.table}>
                ...
                <tfoot>
                  <tr style={{ background: "rgba(220,38,38,0.08)", borderTop: "2px solid rgba(220,38,38,0.3)" }}>
                    <td colSpan={8} className={styles.td} style={{ fontWeight: 800, color: "#f1f5f9", fontSize: "0.83rem" }}>
                      TOTALS ({filteredData.length} entries)
                    </td>
                    <td className={styles.td} style={{ fontWeight: 800, color: "#fbbf24" }}>{fmtNum(kpis.totalIndentQty)}</td>
                    <td className={styles.td} style={{ fontWeight: 800, color: "#4ade80" }}>{fmtNum(kpis.totalIssueQty)}</td>
                    <td className={styles.td} style={{ color: "#94a3b8" }}>—</td>
                    <td className={styles.td} style={{ fontWeight: 800, color: "#fbbf24" }}>{fmtCurrency(kpis.totalPrice)}</td>
                  </tr>
                </tfoot>
              </table>
            )}
